...
actionService.trigger = (actionID, value, alternativeID) => {
    return new Promise((resolve, reject) => {
        ...
        validateAction(actionID)
            .then(() => {
                const body = createRequestBody(actionID, value, alternativeID);
                BotService.post('/actions', body)
                    ...
                    .catch((error) => {
                        reject(error);
                        logger.error('Failed to trigger action with actionID: '
                            + actionID);
                    });
            })
            .catch((error) => {
...
function createRequestBody (actionID, value, alternativeID) {
    const data = {
    ...
    return new Promise((resolve, reject) => {
        getActionByID(actionID)
            ...
            .catch(() => {
                ...
                reject({statusCode: 404, message: 'Action not found'});
            });
    });
}
