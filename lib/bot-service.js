...
botService.get = async (resource) => {
    return new Promise((resolve, reject) => {
        ...
        logger.debug("GET Request Parameters: " + JSON.stringify(parameters));
...
botService.post = async (resource, data) => {
    return new Promise((resolve, reject) => {
        ...
        logger.debug("POST: Request Parameters: " + JSON.stringify(parameters));
        let request = https.request(parameters, (response) => {
            ...
            response.on('end', () => {
                ...
                const payloadData = decodeToken(payload);
...
function decodeToken (token) {
    try {
        ...
        if (decoded.hasOwnProperty('bot')) {
            logger.debug("Decoded BoT Contents:  " + JSON.stringify(decoded));
            return decoded.bot;
        }
...
function handleErrors (request, reject) {
    request.on('error', (error) => {
        logger.error('A problem has occurred while sending the client request. ' +
            error);
        reject();
    });
}
